        <div class="col-lg-3 col-md-6">
          <div class="left-area">
            ...
            <div class="body-area">
              <form id="catalogForm" action="{{ route('front.category', ['category' => Request::route('category'),'subcategory' =>Request::route('subcategory'),'childcategory' =>Request::route('childcategory'),'lang' => $sign]) }}" method="GET">
                ...
                <ul class="filter-list">
                  ...
                  <li>
                    <div class="content">
                        @if(!$slang)
              @if($lang->id == 2)
              <a href="{{route('front.category', ['category' => $element->slug_ar , 'lang' => $sign ])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="category-link"> <i class="fas fa-angle-double-right"></i>
              {{$element->name_ar}}
              @else 
              <a href="{{route('front.category', ['category' => $element->slug , 'lang' => $sign ])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="category-link"> <i class="fas fa-angle-double-right"></i>
               {{$element->name}}
              @endif 
          @else  
              @if($slang == 2) 
          ...
          @endif</a>
                        @if(!empty($cat) && $cat->id == $element->id && !empty($cat->subs))
                            ...
                            <div class="sub-content open">
                             @if(!$slang)
              @if($lang->id == 2)
               <a href="{{route('front.category', ['category' => $cat->slug_ar ,'subcategory' =>  $subelement->slug_ar, 'lang' => $sign ] )}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="subcategory-link"><i class="fas fa-angle-right"></i>
              {{$subelement->name_ar}}
              @else 
               <a href="{{route('front.category', ['category' => $cat->slug ,'subcategory' =>  $subelement->slug, 'lang' => $sign ])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="subcategory-link"><i class="fas fa-angle-right"></i>
              {{$subelement->name}}
              @endif 
          @else  
              @if($slang == 2) 
          ...
          @endif</a>
                              @if(!empty($subcat) && $subcat->id == $subelement->id && !empty($subcat->childs))
                                ...
                                <div class="child-content open">
                                  @if(!$slang)
              @if($lang->id == 2)
              <a href="{{route('front.category',  ['category' => $cat->slug_ar ,'subcategory' => $subcat->slug_ar ,'childcategory' =>  $childcat->slug_ar, 'lang' => $sign ])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="subcategory-link"><i class="fas fa-caret-right"></i>
              {{$childcat->name_ar}}
              @else 
              <a href="{{route('front.category', ['category' => $cat->slug ,'subcategory' => $subcat->slug ,'childcategory' =>  $childcat->slug, 'lang' => $sign ])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="subcategory-link"><i class="fas fa-caret-right"></i>
             {{$childcat->name}}
              @endif 
          @else  
              @if($slang == 2) 
